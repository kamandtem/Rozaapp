...
export const FaceMasksView: React.FC<{ onClose?: () => void }> = ({ onClose }) => {
  ...
  const filteredMasks = FACE_MASKS_DATABASE.filter((m) => {
    ...
    const matchesSearch =
      search.trim() === '' ||
      m.titleFa.includes(search) ||
      m.summaryFa.includes(search) ||
      m.ingredientsFa.some((i) => i.includes(search));
        ...
        {selectedMask && (
          <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4">
            <motion.div
              ...
              className="w-full max-w-lg max-h-[85vh] overflow-y-auto p-6 rounded-3xl bg-white dark:bg-slate-900 text-right space-y-4 shadow-2xl border border-rose-100 dark:border-slate-800 font-['Vazirmatn',sans-serif]"
            >
