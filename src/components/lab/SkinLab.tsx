...
export const SkinLab: React.FC<SkinLabProps> = ({ initialTab = 'ingredients' }) => {
  ...
  const filteredConditions = SKIN_CONDITIONS_DATABASE.filter(
    (c) =>
      ...
      c.summaryFa.includes(searchQuery) ||
      (c.symptomsFa && c.symptomsFa.some((s) => s.includes(searchQuery)))
  );
  ...
  const getConflictStatus = () => {
    ...
    const conflict1 = ing1.avoidCombining.some((av) => ing2.name.includes(av) || ing2.nameFa.includes(av));
    const conflict2 = ing2.avoidCombining.some((av) => ing1.name.includes(av) || ing1.nameFa.includes(av));
