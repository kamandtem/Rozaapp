...
export function generateDailyRecommendations(
  ...
  if (phase === 'menstrual') {
    ...
    if (cycleInfo.inPmsWindow) {
      ...
      cycleInsight = `فاز لوتئال (دوره PMS). منافذ پوست تحت تاثیر هورمون‌ها تمایل به تورم و انسداد دارند. پیشگیری از جوش اولویت اصلی است.`;
    } else {
      cycleInsight = `فاز لوتئال اولیه. ترشح چربی رو به افزایش است. روتین کنترل چربی و مرطوب‌کننده فاقد چربی توصیه می‌شود.`;
    }
    recommendedIngredients.push('سالیسیلیک اسید (BHA)', 'نیاسینامید', 'عصاره درخت چای', 'آزلائیک اسید');
  ...
  const nightRoutine: RoutineStep[] = [
    ...
    {
      id: 'n_step_2',
      titleFa: phase === 'luteal_pms' ? 'سرم سالیسیلیک اسید (BHA) ضدجوش' : 'سرم ترمیم‌کننده و ضدچروک شب',
      category: 'treatment',
      productNameFa: ownedProducts.find(p => p.category === 'treatment' || p.category === 'serum')?.name || (phase === 'luteal_pms' ? 'سرم سالیسیلیک اسید BHA' : 'سرم نیاسینامید یا هیالورونیک اسید'),
      completed: false,
      timeSeconds: 30,
      descriptionFa: phase === 'luteal_pms' ? 'کنترل چربی منافذ و پیشگیری از جوش‌های هورمونی.' : 'تغذیه عمیق و ترمیم سلولی پوست در طول خواب.'
    },
    {
      id: 'n_step_3',
