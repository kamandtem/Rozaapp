...
export function computeCycleInfo(
  ...
  let phase: 'menstrual' | 'follicular' | 'ovulation' | 'luteal_pms' = 'follicular';
  let phaseNameFa = 'فولیکولار (رشد و شادابی)';
  let inPmsWindow = false;

  const ovulationDay = Math.round(cycleLength / 2); // e.g. Day 14 for 28-day cycle
  ...
  if (cycleDay <= periodLength) {
    ...
    if (daysUntilNextPeriod <= pmsStartDaysBefore) {
      inPmsWindow = true;
      phaseNameFa = 'لوتئال (دوره PMS و جوش هورمونی)';
    } else {
      phaseNameFa = 'لوتئال اولیه (ترشح پروژسترون)';
    }
  }
